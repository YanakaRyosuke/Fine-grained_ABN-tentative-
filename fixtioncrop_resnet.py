...
def main():
    ...
    if args.resume:
        ...
        logger = Logger(os.path.join(args.checkpoint, 'log_fix_resnet_2000_84_tta.txt'), title=title, resume=True)
    else:
        logger = Logger(os.path.join(args.checkpoint, 'log_fix_resnet_2000_84_tta.txt'), title=title)
    ...
    logger.close()
    logger.plot()
    savefig(os.path.join(args.checkpoint, 'log_fix_resnet_2000_84_tta.eps'))
...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint_fix_resnet_2000_84_tta.pth.tar'):
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_fix_resnet_2000_84_tta.pth.tar'))
