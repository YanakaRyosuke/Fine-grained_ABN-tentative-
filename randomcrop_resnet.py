#2021_1_7作製最
#使用モデルbird使用，resnet.py使用
#LSTMなしでvggのみで実装，そもそもLSTMを使う意味は?
#Finetuning
...
def main():
    ...
    if args.resume:
        ...
        logger = Logger(os.path.join(args.checkpoint, 'log_ramdom_42.txt'), title=title, resume=True)
    else:
        logger = Logger(os.path.join(args.checkpoint, 'log_random_42.txt'), title=title)
    ...
    logger.close()
    logger.plot()
    savefig(os.path.join(args.checkpoint, 'log_random_42.eps'))
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        targets_32 = []
        #スライドクロップの種類は224_32,168_44,112_48の3種類
        #3種の場合は32枚生成になるのでrange(32)
        #2種の場合はrange(15),1種の場合はrange(5)
        #ramge(32)の時は256になるけれど大丈夫?
        ...
        for i in range(len(x_numpy)):
            ...
            for j in range(4):
                x_list.append(x)
            targets_32.extend(x_list)
        arr_targets_32 = np.array(targets_32)
        targets = torch.tensor(arr_targets_32).to(device)

        con = []

        for batch_num,(inputs_batch) in enumerate(inputs):
            x_pil = transforms.functional.to_pil_image(inputs_batch.cpu())
            transform_inputs_img = transforms.Resize(256) #pilとしてresize
            transform_random_crop = transforms.RandomCrop(84)
            transform_inputs_img_net = transforms.Resize(216)
            ...
            for random in range(4):
                r_crop_pil = transform_inputs_img(x_pil)
                r_crop_pil = transform_random_crop(r_crop_pil)
                r_crop_pil = transform_randomverticalflip(r_crop_pil)
                r_crop_pil = transform_randomhorizontalflip(r_crop_pil)
                r_crop_pil = transform_inputs_img_net(r_crop_pil)
                r_crop_tensor = transforms.functional.to_tensor(r_crop_pil)
                r_crop_tensor = torch.unsqueeze(r_crop_tensor,0)
                con.append(r_crop_tensor)
...
def test(testloader, model, criterion, epoch, use_cuda):
    ...
    for batch_idx, (inputs, targets) in enumerate(testloader):
        ...
        # compute output
        # compute output
        #outputs, attention = model(resize_crops)
        #import pdb;pdb.set_trace()
...
def save_checkpoint(state, is_best, checkpoint='checkpoint', filename='checkpoint_random_42.pth.tar'):
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(checkpoint, 'model_best_random_42.pth.tar'))
